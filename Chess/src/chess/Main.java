...
public class Main extends JFrame implements MouseListener {
	...
	private Button start, startChess960, wselect, bselect, WNewPlayer, BNewPlayer;
...
//			}
		...
		start.addActionListener(new START(GameType.NORMAL));
		start.setPreferredSize(new Dimension(120, 40));
		startChess960 = new Button("Start Chess 960");
		startChess960.setBackground(Color.black);
		startChess960.setForeground(Color.white);
		startChess960.addActionListener(new START(GameType.CHESS960));
		startChess960.setPreferredSize(new Dimension(120, 40));
	...
	private void setup960Board() {
		...
		ArrayList<Piece> blacks = new ArrayList<>(Arrays.asList(br01, bk, br02, bk01, bk02, bb01, bb02, bq));
		ArrayList<Piece> whites = new ArrayList<>(Arrays.asList(wr01, wk, wr02, wk01, wk02, wb01, wb02, wq));
		Piece p;
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				if (x == 0) {
					int bIndex = rand.nextInt(blacks.size());
					while (previousIndexes.contains(bIndex)) {
						bIndex = rand.nextInt(blacks.size());
					}
					p = blacks.get(bIndex);
					if (!(p instanceof Bishop) && bishopPos > -1 && y > 5 && bishopPos % 2 != y % 2) {
						for (int i = 0; i < blacks.size(); i++) {
							if (!previousIndexes.contains(i) && blacks.get(i) instanceof Bishop) {
								p = blacks.get(i);
							}
						}
					} else if (p instanceof Rook && firstRook && !king) {
						while (p instanceof Rook || previousIndexes.contains(bIndex)) {
							bIndex = rand.nextInt(blacks.size());
							p = blacks.get(bIndex);
						}
					} else if (p instanceof King && !firstRook) {
						while (p instanceof King || previousIndexes.contains(bIndex)) {
							bIndex = rand.nextInt(blacks.size());
							p = blacks.get(bIndex);
						}
					} else if (p instanceof Bishop && bishopPos != -1 && bishopPos % 2 == y % 2) {
						while (p instanceof Bishop || previousIndexes.contains(bIndex)) {
							bIndex = rand.nextInt(blacks.size());
							p = blacks.get(bIndex);
						}
					...
					p = whites.get(previousIndexes.get(previousIndexes.size() - y - 1));
					placePiece(p, x, y);
				} else {
					p = null;
					placePiece(p, x, y);
	...
	class START implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			case NORMAL:
				...
				break;
			default:
				throw new IllegalStateException("unrecognized GameType " + gt.toString());
			}
			...
			displayTime.remove(startChess960);
